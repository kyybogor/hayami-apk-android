...
class _StockdetailState extends State<Stockdetail> {
  ...
  List<String> getUkuranOptions(String query) {
    ...
    final ukuran = bahanList
        .where((item) => item['id_bahan'] == selectedBahan && item['model'] == selectedModel)
        .map((item) => item['ukuran'].toString())
        .toSet()
        .toList();
    return ukuran.where((item) => item.toLowerCase().contains(query.toLowerCase())).toList();
  }
...
Future<void> generateExcel() async {
  ...
  final headers = ['Tanggal Masuk', 'No. Transaksi', 'Model', 'Ukuran', 'Qty', 'Harga'];
  ...
  for (int i = 0; i < filteredStock.length; i++) {
    ...
    sheet.getRangeByIndex(row, 5).setNumber(double.tryParse(item['stock'].toString()) ?? 0);
    sheet.getRangeByIndex(row, 6).setNumber(double.tryParse(item['harga'].toString()) ?? 0);

    for (int col = 1; col <= 6; col++) {
      sheet.getRangeByIndex(row, col).cellStyle.borders.all.lineStyle = xlsio.LineStyle.thin;
    }
  }
  ...
  final path = '${directory!.path}/StockDetail_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  Future<void> fetchDetailStock() async {
    ...
    final url = Uri.parse(
      'http://192.168.1.11/pos/detail_stock.php'
                ...
                children: [
                  Expanded(
                    child: Autocomplete<String>(
                      optionsBuilder: (textEditingValue) => getBahanOptions(textEditingValue.text),
                      ...
                      fieldViewBuilder: (context, controller, focusNode, _) => TextField(
                        ...
                        focusNode: focusNode,
                        decoration: const InputDecoration(labelText: 'ID Bahan', border: OutlineInputBorder()),
                      ),
                    ),
                  ),
                  ...
                  Expanded(
                    child: Autocomplete<String>(
                      optionsBuilder: (textEditingValue) => getModelOptions(textEditingValue.text),
                      onSelected: (selection) {
                        setState(() {
                          selectedModel = selection;
                          selectedUkuran = null;  // Reset ukuran ketika model dipilih
                        });
                      },
                      fieldViewBuilder: (context, controller, focusNode, _) => TextField(
                        controller: controller,
                        ...
                        onChanged: (text) {
                          if (text.isEmpty) {
                            setState(() {
                              ...
                              selectedUkuran = null; // Reset ukuran ketika model kosong
                            });
                          }
                        },
                  ...
                  Expanded(
                    child: Autocomplete<String>(
                      optionsBuilder: (textEditingValue) => getUkuranOptions(textEditingValue.text),
                      onSelected: (selection) {
                        setState(() {
                      ...
                      fieldViewBuilder: (context, controller, focusNode, _) => TextField(
                        ...
                        onChanged: (text) {
                          if (text.isEmpty) {
                            setState(() {
                              selectedUkuran = null;  // Reset ukuran ketika kosong
                            });
                          }
                        },
                        decoration: const InputDecoration(labelText: 'Ukuran', border: OutlineInputBorder()),
                      ),
                    ),
                  ),
                ],
              ...
              Row(
                children: [
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(backgroundColor: Colors.indigo.shade200, foregroundColor: Colors.white),
                    child: const Row(
                      children: [Icon(Icons.print, color: Colors.white), SizedBox(width: 8), Text('Print Excel')],
                    ),
                  ),
                ],
              ),
                  ...
                  children: [
                    TableRow(
                      ...
                      children: const [
                        ...
                        Padding(padding: EdgeInsets.all(8), child: Text('Stock', style: TextStyle(color: Colors.white))),
                      ],
                    ),
                    TableRow(
                      children: [
                        Padding(padding: const EdgeInsets.all(8), child: Text(stockDetail!['id_bahan'])),
                        Padding(padding: const EdgeInsets.all(8), child: Text(stockDetail!['total_stock'].toString())),
                      ],
                    ),
                  ],
                ),
  ...
  children: [
    TableRow(
      decoration: const BoxDecoration(color: Colors.indigo),
      children: const [
        Padding(padding: EdgeInsets.all(8), child: Text('Tanggal Masuk', style: TextStyle(color: Colors.white))),
        Padding(padding: EdgeInsets.all(8), child: Text('No. Transaksi', style: TextStyle(color: Colors.white))),
        Padding(padding: EdgeInsets.all(8), child: Text('Model', style: TextStyle(color: Colors.white))),
        ...
        Padding(padding: EdgeInsets.all(8), child: Text('Harga', style: TextStyle(color: Colors.white))),
        Padding(padding: EdgeInsets.all(8), child: Text('Action', style: TextStyle(color: Colors.white))),
      ],
    ),
    for (final item in filteredStock)
      TableRow(
        children: [
          Padding(padding: const EdgeInsets.all(8), child: Text(item['tgl_masuk'])),
          Padding(padding: const EdgeInsets.all(8), child: Text(item['id_transaksi'])),
          Padding(padding: const EdgeInsets.all(8), child: Text(item['model'])),
          Padding(padding: const EdgeInsets.all(8), child: Text(item['ukuran'])),
          Padding(padding: const EdgeInsets.all(8), child: Text(item['stock'].toString())),
          Padding(
            padding: const EdgeInsets.all(8),
            child: Text(
              NumberFormat.currency(locale: 'id_ID', symbol: 'Rp ', decimalDigits: 0).format(double.tryParse(item['harga'].toString()) ?? 0),
            ),
          ),
          Padding(
            padding: const EdgeInsets.all(8),
  ...
  for (int i = 0; i < totalPages; i++) {
    ...
    final qtyThisPage = isLast && (totalStock % perQty != 0) ? (totalStock % perQty) : perQty;
    final qtyFormatted = qtyThisPage.toString().padLeft(2, '0');
    final kodeBarcode = item['barcode'].toString() + qtyFormatted;
    final svg = barcode.toSvg(kodeBarcode, width: 200, height: 80);

    pdf.addPage(
      pw.Page(
        build: (context) {
          return pw.Center( // <--- memastikan semua di tengah halaman
            child: pw.Column(
              mainAxisSize: pw.MainAxisSize.min,
              children: [
                pw.Text('${stockDetail?['id_bahan'] ?? '-'}  x$qtyFormatted',
                    style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
                pw.Text('${item['model']} ${item['ukuran']}',
